...
class BaseDetailSeprationBatch(DatasetBatch, CharacterBatch):
    def __init__(self, parameters, view, name="BaseDetailSepration", dataset_name="3dmodel"):
        ...
        self._parameters["sigmaSpace"].valueChanged.connect(self._computeDetailLayer)
        self._parameters["sigmaRange"].valueChanged.connect(self._computeDetailLayer)
        self._parameters["bumpScale"].valueChanged.connect(self._computeBumpNormal)
        self._view = view
    ...
    def _computeDetailLayer(self):
        ...
        self._D = D_b
        self._N_b = bumpNormal(self._D, scale=1.0, sigma=1.0)
        self._view.render(D_b)

    def _computeBumpNormal(self):
        bump_scale = self._parameters["bumpScale"].value()
        N0_32F = np.array(self._N_b)
        N0_32F[:, :, :2] *= bump_scale
        N0_32F = normalizeImage(N0_32F, th=1.0)
        self._view.render(normalToColor(N0_32F))
        self._N0_32F = N0_32F

    def computeLowFrequencyNormal(self):
        th = 0.1
        N0_32F = self._N0_32F
        h, w = N0_32F.shape[:2]
        W_32F = np.zeros((h, w))
        ...
        self._view.render(normalToColor(N_32F, self._A_8U))

        self._N_b_smooth = N_32F

    def computeLumoNormal(self):
        A_8U = self._A_8U
        ...
        N_32F = bumpMapping(N_32F, self._N_b_smooth)
...
#             self._runLayer(layer_file)
    ...
    def _runLayer(self, layer_file):
        ...
        self._loadImage()
        self._computeDetailLayer()
        self._computeBumpNormal()
        self.computeLowFrequencyNormal()
        self.computeLumoNormal()

        # plt.savefig(self.characterResultFile("BumpNormal.png"))

        if self._N_32F is not None:
            saveNormal(self.characterResultFile("BumpNormal.png"), self._N_b, A_8U)
            saveNormal(self.characterResultFile("BumpSmoothNormal.png"), self._N_b_smooth, A_8U)
            saveNormal(self.characterResultFile("SilhouetteNormal.png"), self._N_lumo, A_8U)
            saveNormal(self.characterResultFile("InitialNormal.png"), self._N_32F, A_8U)
...
if __name__ == '__main__':
    ...
    editor.addButton("Compute Silhouette Normal", cmd_func=batch.computeLumoNormal)
    editor.addButton("Compute Detail Normal", cmd_func=batch.computeLowFrequencyNormal)
